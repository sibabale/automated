...
describe('overview proxy route', () => {
    it('rejects a request without a ticker and preserves the correlation id header', async () => {
        const response = await GET(request('http://localhost/api/overview'));
    ...
    it('forwards the ticker and correlation id to the backend and returns its response unchanged', async () => {
        ...
        const response = await GET(request('http://localhost/api/overview?ticker=aapl'));
        ...
        await expect(response.json()).resolves.toMatchObject({ data: { reportHeader: { ticker: 'AAPL' } } });
    });
