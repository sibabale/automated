// [ APP > API > ANALYSIS > PROFIT MARGIN > TESTS ] ##################################################
...
describe('profit margin proxy route', () => {
    it('rejects a request without a ticker and preserves the correlation id header', async () => {
        const response = await GET(request('http://localhost/api/analysis/profit-margin'));
    ...
    it('forwards the ticker and correlation id to the backend and returns its response unchanged', async () => {
        ...
        const response = await GET(request('http://localhost/api/analysis/profit-margin?ticker=aapl'));
